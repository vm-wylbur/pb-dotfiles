...
#+end_src
** Backups
So, keep a lot of backups, but do it out of focus.
#+begin_src emacs-lisp
(setq
...
=./autogen.sh=. Of course there's a challenge: =brew='s installer for
the mu package wants a hardlink to the emacs it's going to
use. Here'sa  workaround: =EMACS=$(which emacs) brew install mu
--with-emacs --HEAD=. It's *not* great.
