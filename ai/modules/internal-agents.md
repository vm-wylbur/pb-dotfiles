### Three internal agents on porky

hrdag-ansible has three internal agents that share the porky controller and
the hardware-backed SSH key (`id_ed25519_sk_porky`):
...
cc-tfc-ingest-gui / cc-tfccc) PR into the same gate. Full per-agent reach
+ workflow rules in `docs/cross-repo-agent-workflow.md` (v2.1).
